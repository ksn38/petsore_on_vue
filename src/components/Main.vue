<template>
  ...
  <main>
    <div v-for="(product, key) in sortedProducts" v-bind:key="key">
      <div class="row">
        <div class="col-md-5 col-md-offset-0">
          <figure><img class="product" v-bind:src="product.image" ></figure>
        </div>
        <div class="col-md-6 col-md-offset-0 description">
          ...
          v-for="(n, key) in 5" v-bind:key="key">☆
          </span>
          </div>
        </div><!-- end of col-md-6-->
      </div><!-- end of row-->
  ...
  computed: {
    ...
    sortedProducts() {
      if (this.products.length > 0) {
        ...
        return productsArray//.sort((a,b) => b[1] - a[1]);
      } else {
        return 1
      }
    }
